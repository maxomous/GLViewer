...
void Camera_CentreObject::SetViewMode(ViewMode mode)
{
    m_ViewMode = mode;
    if(mode == ViewMode::View2D) { // 2d mode
        m_Perspective = false;
        m_Yaw = 90.0f;
        m_Pitch = 90.0f;
    } 
    else if(mode == ViewMode::View3D) { // 3d mode
        m_Perspective = true;
        m_Yaw = m_YawDefault;
        m_Pitch = m_PitchDefault;
    } 
    else { assert(0 && "View Mode Unknown"); }
    updateCameraVectors();
}
...
{    
    ...
    float invertFor2DMode = (m_ViewMode == ViewMode::View2D) ? -1.0f : 1.0f;
